...
namespace engine
{
	namespace input
	{
		namespace sdl
		{
			...
			{
			public:
				SdlJoystick(InputGetMode newMode = InputGetMode::POOL_INPUT, size_t timeOut_ms = 0);
				~SdlJoystick();

				void PoolInput() override; // TO_DO Usar apenas PullEvent ou usar PushEvent também de alguma maneira caso o evento não for ser tratado pelo jogo mas for um evento importante como o quit por exemplo?
